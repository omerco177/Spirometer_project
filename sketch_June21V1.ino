...
void loop()
{
  ...
  while (CalibFlag == 0 && GameMode == 1) // Game
    {
        ...
        // Read the analog value from the sensor
        //startGameTime = millis();
        //while(GameTimeInSeconds <= GAME_LUNGE_TIME)
        //{
          PartControler();
        ...
        //DisplayCalibProcess(2,0,(LungVolume),"COSEMEK","LungVolume:",500);
   
        //normalizedValue_Game =(LungVolume / MAXVOLUME) * 9.0;
       // DisplayFloatMessageScreen(2,0,normalizedValue_Game,"normalizedValue",1000);
        //normalizedValue_Game = 2*normalizedValue_Game;
       // DisplayFloatMessageScreen(2,0,normalizedValue_Game,"normalizedValue",1000);


        //DisplayCalibProcess(2,0,(normalizedValue),"COSEMEK","LungVolume:",500);

        Serial.print("Voll: \n"); // Print the pressure value to the serial monitor
        Serial.print(normalizedValue);
        Serial.println(" litter\n");
        //DisplayIntMessageScreen(2,1,10*LungVolume,"Volume:");
        //WORKS -OMER GIL
        //lcd.begin(16, 2);
        //lcd.setCursor(8,1 );
        ...
        //delay(2000);
        //lcd.clear();
        if (((Zero_stage + Delta_stage ) <= LungVolume) && (LungVolume<= Third_stage + Delta_stage) && stage1_counter< STAGE_REP_LIMIT)   
        {
           Serial.println("First Stage\n");
           //TODO - ADD LCD SCREEN
           //DisplayStage(2,"First Stage",Zero_stage,Third_stage);
           DisplayStage3Num(2,"First Stage",LungVolume,Zero_stage,Third_stage);
           stage_state = 1;
           if ((First_stage  < LungVolume) && (LungVolume <= Third_stage ) &&(LungVolume != 0))
           {
            lcd.clear();
            //DisplayFloatMessageScreen(2,0,LungVolume,"PLV",400);           
            stage1_counter++;
            //DisplayMessageScreen(2,0,"In range",400);                
            stage_global_counter++;
            delay(300);
           } 
          Game_Enable();
        }
        else if ((First_stage + Delta_stage<= LungVolume) && (LungVolume  <= Third_stage - Delta_stage) && stage2_counter< STAGE_REP_LIMIT)   
        {
           lcd.clear();          
           Serial.println("Second Stage\n");
           stage_state = 2;
           //TODO - ADD LCD SCREEN
           //DisplayStage(2,"Second Stage",First_stage,Third_stage);
           DisplayStage3Num(2,"Second Stage",LungVolume,First_stage,Third_stage);
           if ((Second_stage-Delta_stage< LungVolume) &&(LungVolume <= Third_stage+Delta_stage))
           {
            lcd.clear();
            //DisplayFloatMessageScreen(2,0,LungVolume,"PLV",400);  
            stage2_counter++;
        ...
        else if ((Second_stage - Delta_stage<= normalizedValue) && (normalizedValue<= Third_stage + Delta_stage) && stage3_counter< STAGE_REP_LIMIT)   
        {
           lcd.clear();          
           Serial.println("Second Stage\n");
           stage_state = 3;
           //TODO - ADD LCD SCREEN
          //DisplayStage(2,"Third Stage",0.9*Third_stage,Third_stage);
           DisplayStage3Num(2,"Third Stage",normalizedValue,0.8*Third_stage,Third_stage);
           if (0.65*Third_stage<= normalizedValue)
           {
            lcd.clear();
            //DisplayFloatMessageScreen(2,0,LungVolume,"PLV",400);               
            stage3_counter++;
        ...
        else if (stage_global_counter == STAGE_TOTAL_LIMIT)
        {
          stage_state = END_TREATMENT;
          DisplayMessageScreen(2,0,"The End",400);               
        }      
        else
        {
         stage_state = 0;
        ...
        //DisplayCalibProcess(2,0,(normalizedValue*100),"Buttons disabled","Time remain:",200);
        //Serial.println("normalizedValue\n");
        //Serial.println(normalizedValue);

         //DisplayMessageScreen(2,0,"Out of range",400);                
        }

        //DisplayFloatMessageScreen(2,0,LungVolume,"PLV",400);  

        //current_stage = round(stage_global_counter/STAGE_REP_LIMIT);
        //DisplayFloatMessageScreen(2,0,LungVolume,"vol",200);               
        //if (stage_state == 0){
          //Enable = 0;
        //}
        //else  
       ...
       // }
        ...
        //}
        LungVolume = 0;
    if (GameMode == 0 ) // No calib////////////////////////////////////////////////CHANGE "0" in IF
    {
            DisplayMessageScreen(2,0,"No Calib", 100);
            int xVal = analogRead(joystickX);
            int yVal = analogRead(joystickY);
            int swVal = digitalRead(joystickSW);
            int buttonSVal = digitalRead(buttonSPin);
            // Map X axis to arrow keys
            PushX(xVal);
            // Map Y axis to arrow keys
            PushY(yVal);
  ...
  //Serial.print(CountLow);
  //Serial.print("\n");
  //Serial.print("SumHigh");
  //Serial.print(SumHigh);
  //Serial.print("CountHigh");
  //Serial.print(CountHigh);
  //int ButtomB = digitalRead(OPTIONS_PIN);
  //Serial.print(ButtomB);
  //Serial.print("\n");
